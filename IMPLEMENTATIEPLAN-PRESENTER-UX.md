...
- [x] **A1 Gum.** Nieuwe toolcategorie `eraser` met borstelgroottes S/M/L (diameter 12/28/56 boardunits). Doorontwikkeld tot precisiegum (18 juli 2026): wist alleen het geraakte stuk en splitst de streek (`erasePartialStrokes`, met segmentverdichting zodat ook rechte lijnen deelbaar zijn). Oorspronkelijk: wist hele pen-/markeerstiftstreken bij raakvlak (stroke-hittest tegen borstelcirkel langs de bewegingslijn); objecten blijven ongemoeid. Lib: `presenterEraser.js` (`findStrokeIdsHitByEraser`) + tests; gebruikt bestaand `removeStrokeFromPresenterPage`-patroon (batch: `removeStrokesFromPresenterPage` toevoegen aan `presenterModel.js`). Cursor: cirkel-outline op borstelgrootte. Undo werkt via bestaande per-pagina history.
