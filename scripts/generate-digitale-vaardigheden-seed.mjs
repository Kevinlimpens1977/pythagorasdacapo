...
import {
  MIN_ITEMS,
  assertCorrectPositionsVary,
  assertHasClosedItems,
  assertTypeFitsPrompt,
  buildOpenAnswer,
  buildOptions,
  inferItemType
} from './lib/vraagItems.mjs';
...
// De vraagregels (welk type past bij welke vraag, hoeveel opties, wat een open
// vraag nodig heeft) staan in scripts/lib/vraagItems.mjs, zodat elk vak dezelfde
// lat hanteert. Wat hier blijft is DV-eigen: feedback die over paragrafen heen
// niet mag herhalen, en de plusparagraaf die geen toetsvraag mag krijgen.
...
const buildNiveau = ({ niveau, chapters, enrichment }) => {
  ...
  const makeQuestionItems = (code, vragen, totalTokens, type, verbodenLeerdoelen) => {
    ...
    assertCorrectPositionsVary(correctPositions, `${niveau} ${code}`);
    assertHasClosedItems(items, `${niveau} ${code}`);
