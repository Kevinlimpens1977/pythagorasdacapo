...
Per hoofdstuk een bestand `h<n>.mjs` met de structuur en de eigenlijke lesstof, gebouwd met `p()` en `checkpoint()` uit `../helpers.mjs`. Lees de kop van dat bestand voordat je begint. De verrijking (leerdoelen, kernbegrippen, voorbeelden, samenvattingen en alle vragen) hoort in `scripts/seed-verrijking/kb/`.
